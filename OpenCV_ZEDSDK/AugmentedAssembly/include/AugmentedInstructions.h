...
extern "C" {
    ...
    public:
        AugmentedInstructions(std::vector<std::vector<std::vector<cv::Point>>>& corners, cv::Mat& camera_image_left, cv::Mat& camera_image_right,
                              std::mutex& mutex, AssemblyStates& assembly_state, std::vector<unsigned>& step_indices, unsigned& parts_cnt,
                              int& keypoints_size);
        int& m_keypoints_size;
    ...
    private:
        ...
        //inline cv::Mat GetLeftFrame() { return m_image_camera_left; }
        //inline cv::Mat GetRightFrame() { return m_image_camera_right; }

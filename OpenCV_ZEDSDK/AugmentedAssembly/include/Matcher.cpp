...
Matcher::Matcher(Method method, const float ratio_thresh): m_ratio_thresh(ratio_thresh), m_method(method)
{
    ...
    {
        m_matcher_sift = cv::DescriptorMatcher::create(cv::DescriptorMatcher::BRUTEFORCE_HAMMING);
    }
                    ...
                    const std::vector<cv::KeyPoint>& keypoints_scene, std::vector<cv::DMatch>& good_matches)
{
    ...
    //FilterOutliersSpatial(keypoints_scene, good_matches);
}
...
void Matcher::FilterOutliersSpatial(const std::vector<cv::KeyPoint>& keypoints_scene, std::vector<cv::DMatch>& good_matches)
{
    ...
    //good_matches = filtered       ---> have to workaround this somehow
}
